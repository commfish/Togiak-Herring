...
rm (C)
#------------------------------------------------------------------------------
# Figure 1aa: Survey- and model-estimated aerial survey biomass (tons).
#------------------------------------------------------------------------------
x<-subset(FIGDATA, select=c(Year, tot_obs_aerial_tons,tot_mat_B_tons, tot_obs_aerial_tuned_tons, threshold))
Y<-c(max(x$Year)+1)
B<- data.frame(Y)  
B["Year"]<-B$Y
B["tot_obs_aerial_tons"]<-0
...
B["tot_obs_aerial_tuned_tons"]<-0
B["threshold"]<-35000
B<-subset(B, select=c(Year, tot_obs_aerial_tons,tot_mat_B_tons,tot_obs_aerial_tuned_tons, threshold))
C<-rbind(x,B) #to create x axis with a greater x value than max(Year)
C[C==0] <- NA

x<-subset(FIGDATA, select=c(Year))
wt_aerial<-A$wt_aerial#forecast mature biomass (tons)
D<-cbind(x,wt_aerial)
Y<-c(max(x$Year)+1)
E<- data.frame(Y)
E["Year"]<-E$Y
E["wt_aerial"]<-0
E<-subset(E, select=c(Year, wt_aerial))
E<-rbind(D,E) 
C<-merge(E, C, by="Year") 
...
Year<-c(max(x$Year)+1)
G<- data.frame(Year) 
for_mat_B_st<-A$for_mat_B_st#forecast mature biomass (tons)
H<-cbind(for_mat_B_st,G)
C<-merge(C,H, by="Year", all=TRUE)
options(scipen=999)
...
g2<-ggplot(data=C, aes(x=Year, y=tot_obs_aerial_tons)) + geom_line(lwd=1, col="black", lty=2) +
  theme(axis.text.x = element_text(size=12,colour="black",family="Times New Roman", angle=90),
  ...
  theme(strip.text.x = element_text(size=12,face="bold"))+ #observed
 theme(text=element_text(family="Times New Roman"))
g2<-g2 + ylab("Biomass (tons)")+xlab("Year") + 
  scale_y_continuous(labels=comma, limits=c(0, 500000)) +
  scale_x_continuous(breaks=seq(min(C$Year),max(C$Year),1)) 

g2<-g2+geom_line(aes(y=tot_mat_B_tons),lwd=2,col="black") + #predicted
       geom_point(aes(y=tot_obs_aerial_tons),pch=21,col="black", bg="blue") +
       geom_point(aes(y=tot_obs_aerial_tuned_tons, size=wt_aerial),pch=21,col="black", bg="green") +
       labs(size = "aerial weights")+ 
       theme(legend.justification=c(1,-1.7), legend.position=c(1,0))+
       geom_point(aes(y=for_mat_B_st), pch=8, col="black",cex=3, lwd=2) +
       geom_line(aes(y=threshold),lwd=2,col="grey") #threshold       

png(file='figures/Figure 1aa.png', res=200, width=8, height=5, units ="in")  
grid.newpage()
pushViewport(viewport(layout=grid.layout(1,1)))
vplayout<-function(x,y) viewport (layout.pos.row=x, layout.pos.col=y)
print(g2,vp=vplayout(1,1:1)) 
dev.off()
...
#------------------------------------------------------------------------------
# Figure 1aaNR: Survey- and model-estimated aerial survey biomass (tons) for NR.
#------------------------------------------------------------------------------
x<-subset(FIGDATA, select=c(Year, tot_obs_aerial_tons,tot_mat_B_tons, tot_obs_aerial_tuned_tons, threshold))
Y<-c(max(x$Year)+3)
B<- data.frame(Y)  
...
g2<-ggplot(data=C, aes(x=Year, y=tot_obs_aerial_tons))+
# geom_line(lwd=1, col="black", lty=2) +
...
g2<-g2 + ylab("Biomass (tons)")+xlab("Year") + 
  scale_y_continuous(labels=comma, limits=c(0, 500000)) +
  scale_x_continuous(breaks=seq(min(C$Year),max(C$Year),10)) 

g2<-g2+geom_line(aes(y=tot_mat_B_tons),lwd=2,col="black") + #predicted
  geom_point(aes(y=tot_obs_aerial_tons),pch=21,col="black", bg="green") +
  geom_point(aes(y=tot_obs_aerial_tuned_tons, size=wt_aerial),pch=21,col="black", bg="green") +
  labs(size = "aerial weights")+ 
  theme(legend.justification=c(1,-1.7), legend.position=c(1,0))+
  geom_point(aes(y=for_mat_B_st), pch=8, col="black",cex=3, lwd=2)+ 
...
png(file='figures/Figure 1aaNR.png', res=200, width=8, height=5, units ="in")  
grid.newpage()
pushViewport(viewport(layout=grid.layout(1,1)))
vplayout<-function(x,y) viewport (layout.pos.row=x, layout.pos.col=y)
print(g2,vp=vplayout(1,1:1)) 
...
windowsFonts(A = windowsFont("Times New Roman"))
png(file='figures/Figure 1b.png', res=200, width=8, height=5, units ="in")  
op <- par(family = "Times New Roman")
...
lines(x$Year,x$tot_obs_aerial_tons,lty=2,lwd=1,col="grey") #observed
...
legend("topright",c("Survey-estimated aerial biomass",
       ...
       col=c("blue", "green", "black","black","blue","purple", "purple","red",
             "red", "orange", "orange","grey"), cex=0.75, bty="n",
       lwd=c(1,1,1,NA,NA,1,NA,1,NA,1,NA,2))
dev.off()
Figure_1b_data<-x
rm (x)

#------------------------------------------------------------------------------
# Figure 1c: Survey- and model-estimated aerial survey biomass (tons) with past forecasts for news release.
#------------------------------------------------------------------------------
y<-A$for_mat_B_st#forecast mature biomass (tons)
x<-subset(FIGDATA, select=c(Year, tot_obs_aerial_tons,tot_mat_B_tons, tot_obs_aerial_tuned_tons,
                            past_forecasts,threshold))
x[x==0] <- NA

windowsFonts(A = windowsFont("Times New Roman"))
png(file='figures/Figure 1c.png', res=200, width=8, height=5, units ="in")  
op <- par(family = "Times New Roman")
yticks <- seq(0, 500000, 50000)
#plot(x$Year,x$tot_mat_B_tons,pch=21,col="black",bg="blue",cex=1.0,lwd=1,
#ylab="Biomass (tons)",xlab="Year", 
#cex.axis=1.2,cex.lab=1.2, family="A",ylim=c(0, 500000),xaxt="n")
plot(x$Year,x$tot_mat_B_tons,pch=21,col="black",bg="black",cex=0.2,lwd=1,
     ylab="Biomass (tons)",xlab="Year", 
     cex.axis=1.2,cex.lab=1.2, family="A",ylim=c(0, 500000),xaxt="n")
...
#points(x$Year,x$tot_obs_aerial_tons,pch=21,col="black", bg="blue", cex=1) #observed
#points(x$Year,x$tot_obs_aerial_tuned_tons,pch=21,col="black", bg="green", cex=1) #observed
points(max(x$Year)+1,y,pch=8,col="black",cex=1)##forecast
points(x$Year,x$past_forecasts,pch=17,col="black",cex=1.3)##past_forecast
lines(x$Year+1,x$threshold,col="grey",lty=1, lwd=2)
axis(side=1,at=seq(min(x$Year),max(x$Year)+1,1),cex.axis=1, las=2)
#legend("topright",c("Survey-estimated aerial biomass",
#                    "Survey-estimated aerial biomass (tuned to model)",
#                    "2018 Model-estimated",
#                    "2018 forecast","Past Forecasts",
#                    "Threshold"),
#       pch=c(16,16, NA, 8, 17,NA),
#       lty=c(NA,NA,1,NA,NA,1), 
#       col=c("blue", "green", "black","black","black","grey"),bg="black", cex=0.75, bty="n",
#       lwd=1)
legend("topright",c("2018 Model-estimated",
                    "2018 forecast","Past Forecasts",
                    "Threshold"),
       pch=c(NA, 8, 17,NA),
       lty=c(1,NA,NA,1), 
       col=c("black","black","black","grey"),bg="black", cex=0.75, bty="n",
       lwd=1)
dev.off()
Figure_1c_data<-x
rm (x)
...
B<-subset(FIGDATA, select=c(Year, tot_mat_B_tons, tot_obs_catch_tons))
B["Catch"]<-B$tot_obs_catch_tons
B["postfishery"]<-B$tot_mat_B_tons-B$tot_obs_catch_tons
...
g4a <- ggplot() +geom_bar(data=B, mapping=aes(x=Year, y=value, fill=variable),stat="identity")+
  ylab("Biomass (tons)")+xlab("Year")+scale_fill_manual(values=c("#E69F00", "#0072B2"))

g4a<-g4a+theme(panel.grid.major = element_blank(),
               panel.grid.minor = element_blank())+  
  theme(axis.text.x = element_text(size=12,colour="black", family="Times New Roman"),
        axis.title.x = element_text(size=14, colour="black",family="Times New Roman"))+
  theme(axis.text.y = element_text(size=12,colour="black",family="Times New Roman"),
...
g4a<-g4a+coord_cartesian(ylim=c(0,maxY))+scale_x_continuous(breaks=seq(min(B$Year),max(B$Year+1),1))+                  
  theme(legend.position="none")
...
library(grid)
g4<-arrange_ggplot2(g4a, g4c,g4b, ncol=1)
dev.off()
...
#------------------------------------------------------------------------------
#detach(package:Hmisc, unload = TRUE)
MAXy<-max(FIGDATAAGE$for_mat_baa_st,na.rm=TRUE)*1.5 # set y max. limit
FIGDATAAGE["for_mat_baa_st"]<-as.numeric(FIGDATAAGE$for_mat_baa_st)
FIGDATAAGE$for_mat_baa_st<-round2(FIGDATAAGE$for_mat_baa_st)
...
g8<-ggplot(FIGDATAAGE, aes(x=Age2, y=for_mat_baa_st)) +  
  ylab("Forecasted Biomass (tons)")+
  xlab("Age")+
  theme(text=element_text(family="Times New Roman", size=12))+
  geom_bar(stat="identity",position="dodge" ,fill="#E69F00")

g8<-g8+geom_text(data=FIGDATAAGE,aes(label=comma(for_mat_baa_st)), size=4,
...
C<-subset(FIGDATAAGE, select=c(Age2, for_mat_baa_st))
Figure_8_data<-C
rm(MAXy,g8)
#------------------------------------------------------------------------------
# Figure 8a: Projected mature biomass at age (tons) for forecast year in percent.
#-------#detach(package:Hmisc, unload = TRUE)
...
g8<-ggplot(FIGDATAAGE, aes(x=Age2, y=for_mat_b_prop*100)) +  
  ylab("Forecasted Percent Biomass")+
  xlab("Age")+
  theme(text=element_text(family="Times New Roman", size=12))+
  geom_bar(stat="identity",position="dodge" ,fill="#E69F00")

g8<-g8+coord_cartesian(ylim=c(0,25))
...
g8<-g8 + theme(legend.position="none")+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))
g8<-g8+geom_text(aes(x = 4, y=24,
                     label=paste("Forecasted weight of the mature population=",for_mat_weighted,"g", sep=""), cex=0.1),
                 family="Times New Roman")
#g8<-g8+geom_text(aes(x = 4, y=0.22,
#                     label=paste("Forecasted weight of the purse seine harvest=",for_seine_weighted,"g", sep=""), cex=0.1),
#                 family="Times New Roman")

png(file='figures/Figure 8a.png', res=200, width=6, height=4, units ="in")
pushViewport(viewport(layout=grid.layout(1,1)))
vplayout<-function(x,y) viewport (layout.pos.row=x, layout.pos.col=y)
print(g8,vp=vplayout(1,1:1))
dev.off()

C<-subset(FIGDATAAGE, select=c(Age2, for_mat_baa_st, for_mat_b_prop))
Figure_8_data<-C
rm(MAXy,g8)
-----------------------------------------------------------------------

#------------------------------------------------------------------------------
# Figure 9a: Projected percentage of mature numbers at age for forecast year with mean WAA
...
g9<-g9+coord_cartesian(ylim=c(0,30))+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))
g9<-g9+geom_text(aes(x = 4, y=max(C$Percentage2*130),
    label=paste("Forecasted weight of the mature population=",for_mat_weighted,"g", sep=""), cex=0.1),
    family="Times New Roman")
#g9<-g9+geom_text(aes(x = 4, y=max(C$Percentage2*120),
#    label=paste("Forecasted weight of the purse seine harvest=",for_seine_weighted,"g", sep=""), cex=0.1),
#    family="Times New Roman")
...
#------------------------------------------------------------------------------
# Figure 9b: Projected percentage of mature numbers at age for forecast year labeled with %AA
# *Check to make sure that the numbers on top of bar sum to 100!!!!
#------------------------------------------------------------------------------
...
#If they don't enter the % manually below
Check<-sum(D$Percentage2)
D["Percentage2"]<-c(0.06,0.11,0.05,0.10,0.16,0.16,0.09,0.09,0.18)
Check
C<-cbind(FIGDATAAGE, D)
C<-cbind(FIGDATAAGE, D)
#Check to make sure that the numbers on top of bar sum to 100!!!!
